...
def extract_roi(atlas,roi):
    atl=atlas.darrays[0].data
...
def calc_dists(atlas,surf):
    ...
    ### load surface
    surfPath=surf
    surffile = nib.load(surfPath) # left hemisphere
    verts = surffile.darrays[0].data.astype('float64')
    faces = surffile.darrays[1].data.astype('int32')
    surf = (verts,faces)
    ...
    for i in range(len(labels)):
        labels[i]=extract_roi(atlas,labels[i])
        
        
    sens_dists=[]
    for i in labels:
        sens_dists.append(analysis.dist_calc(surf,cort,i))
    sens_dists=np.asarray(sens_dists)
...
#     out_dict[subj]=min_dist
    return np.min(sens_dists,axis=0)
